...
execute as @a[scores={spawn_boss=1..}] at @a[scores={spawn_boss=1..}] if entity @s[nbt={SelectedItem:{components:{"minecraft:item_model": "minecraft:purple_dye"}}}]:
    execute positioned ~5 ~2 ~ run function stuff:bosstime
    scoreboard players set @s spawn_boss 0
    clear @s warped_fungus_on_a_stick[item_model="purple_dye"]
